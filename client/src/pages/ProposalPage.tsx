...
export default function ProposalPage() {
  ...
  const moveNoButton = () => {
    // Calculate random position within viewport, but keep it somewhat visible
    // Using fixed pixel offsets from original position to keep it simple but erratic
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center p-4 overflow-hidden relative bg-gradient-to-b from-white to-pink-100">
      ...
      >
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-8 md:gap-16 pt-8 pb-20 relative min-h-[200px]">
          {/* YES BUTTON */}
          <motion.button
          ...
          <motion.div
            ...
            className="absolute md:static" // Absolute on mobile to let it fly everywhere, static layout on desktop initially
            style={{ 
              // Ensure it doesn't block the Yes button initially
              zIndex: 10 
            }}
          >
            <button
              onMouseEnter={moveNoButton}
              onTouchStart={(e) => { e.preventDefault(); moveNoButton(); }} // Mobile touch support
              className="px-8 py-4 bg-gray-200 text-gray-500 text-xl font-bold rounded-full shadow-inner cursor-not-allowed whitespace-nowrap"
            >
              {isHoveringNo ? "Catch me!" : "Ummmmm..."}
            </button>
          </motion.div>
        </div>
      </motion.div>
